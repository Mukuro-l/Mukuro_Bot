...
while True:
    if select == "y":
        ...
        if the_ad == "aarch64":
            print("检测到系统为linux arm64")
            print("正在下载对应版本的go-cqhttp")
            subprocess.getoutput("sudo wget https://cdn.githubjs.cf/Mrs4s/go-cqhttp/releases/download/v1.0.0-rc3/go-cqhttp_linux_arm64.tar.gz")
            print("已成功下载")
            ...
            subprocess.getoutput("cd "+str(os.path.dirname(file_path))+"/gocq/ && sudo tar -xzf go-cqhttp_linux_arm64.tar.gz && sudo rm go-cqhttp_linux_arm64.tar.gz")
