...
use Swoole\Coroutine;
	...
	if (@$Data['meta_event_type'] !== 'heartbeat' && @$Data['meta_event_type'] !== 'lifecycle') {
		...
		//这里进行载入为了避免不必要的数据
				...
				$submit_data = json_encode($url, JSON_UNESCAPED_UNICODE);
				$this->ws->push($frame->fd, $submit_data);
				}
				continue;
				}else{
				if ($Data['message_type']==="group"){
				...
				$ws->push($submit_data);
					...
					if (json_decode(file_get_contents("./Group/".$Data['group_id']."/config.json"),true)["status"] === "开" || $Data['msg_type'] === 'private'){
					$file_array = json_decode(file_get_contents("Plugins_switch.json"), true);
					...
					//创建协程
						...
						Coroutine::create(function () use ($barrier,$list,$file,$Plugins_name,$file_array,$Data, $database, $BOT_Config,$ws){
